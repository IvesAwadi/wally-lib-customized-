...
finity.dark_theme = { -- dark
	...
	dropdown_background = Color3.fromRGB(60, 60, 60),
	dropdown_text = Color3.fromRGB(220, 220, 220),
				...
				if kind then
                    ...
                    elseif string.lower(kind) == "color" or string.lower(kind) == "colorpicker" then
                        ...
                        cheat.hsvbar.MouseButton1Down:Connect(function()
                            ...
                            local uis = game:GetService("UserInputService")local last = cheat.value;
					...
					elseif string.lower(kind) == "dropdown" then
						...
						function cheat.fadelist()
							...
							if cheat.dropped then
								...
								game:GetService("TweenService"):Create(cheat.list, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, math.clamp(cheat.list["UIListLayout"].AbsoluteContentSize.Y, 0, 150)), Position = UDim2.new(0, 0, 1, 0), ScrollBarImageTransparency = 0, BackgroundTransparency = 0.05}):Play()
							else
								for _, button in next, cheat.list:GetChildren() do
									if button:IsA("TextButton") then
